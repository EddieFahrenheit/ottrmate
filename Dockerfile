...
RUN adduser --system --uid 1001 ottrmate

# Copy all these files from the builder section up above, into the container
COPY --from=builder /app/public ./public
COPY --from=builder --chown=ottrmate:nodejs /app/.next/standalone ./
COPY --from=builder --chown=ottrmate:nodejs /app/.next/static ./.next/static
